...
namespace ConfusedDevelopProject.Core
{
    ...
    public abstract class SqlHelper
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static int GetAllBookCount(string sql)
        {
            int num = 0;
            int pageCount = 0;
            string sqlCount = string.Format(@"select count(0) from ({0}) as a", sql);
            num = (int)ExecuteScalar(sqlCount);
        ...
        public static TEntity ExecuteReader<TEntity>(string sql)
        {
            SqlConnection con = new SqlConnection(ConnectionStringLocalTransaction);
            con.Open();
            SqlCommand cmd = new SqlCommand(sql, con);
            ...
            using (IDataReader dataReader = cmd.ExecuteReader())
            {
                if (dataReader.Read())
                {
                    result = dataReader.ReaderToModel<TEntity>(null);
                }
            }
            con.Close();
            return result;
        ...
        public static TEntity ExecuteReader<TEntity>(string commandText,
            IDictionary<string, object> parameters = null,
            Action<TEntity> setItem = null) where TEntity : class, new()
        {
        ...
        private static T CreateDbCommondAndExcute<T>(string commandText,
